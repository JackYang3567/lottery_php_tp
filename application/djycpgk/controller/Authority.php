	...
	public function proxy_add() { //添加管理员
		if (Request::method() == 'GET') {
			...
			foreach ($grouplist as $key => $value) {
                ...
                if ($value['id'] ==16) {
                    $grouplist[$key]['jiedian'] = DB::table('think_auth_rule')->where([['type','=',6],['status','=',1]])->field('id,title,type')->select();
                }
			}
			// dump($grouplist);
...
//				 dump(Request::post('group_id/a'));exit();
	...
	public function secondlist() {
		...
		if (Request::param('keywords') != '') {
			$map['username'] = ['username', '=', Request::param('keywords')];
			$pageParam['query']['keywords'] = Request::param('keywords');
            $this->assign('keywords', Request::param('keywords'));
		}
...
//			 dump($child_num);
			...
			if ($child_num != 0) {
                ...
                $ss = [];
                $new_relationship = Db::table('new_relationship')->where('user_id','in',$user_array)->select();//获取 new_relationship 普通会员的 代理关系

                if(count($new_relationship)==0){
                    $item['children_num'] = count($user_array);
                }else{
                    foreach ($new_relationship as $value){
                        $arr[]  = explode(",",$value['child_one']);
                        $arr[]  = explode(",",$value['child_two']);
                        $arr[]	= explode(",",$value['child_three']);
                    ...
                    $item['children_num'] = count(array_unique(array_filter($ss)));  //查询所有二级代理下面的 普通用户 和 普通用户 下面的 二级；
                }
	...
	public function ordinarylist() {
        ...
        $ss = [];
        $new_relationship = Db::table('new_relationship')->where('user_id','in',$ids)->select();//获取 new_relationship 普通会员的 代理关系
		...
		$list = Db::table('user')->where($map)->paginate($pageNum, false,  $config = ['query'=>array('second_id'=>Request::param('second_id'),'top_id'=>Request::param('top_id'))])->each(function ($item, $key) {
			$total_cz = 0;
			$total_tx = 0;
			$total_fs = 0;
			$total_fy = 0;
		...
		$this->assign('keywords', Request::param('keywords'));//名称
		...
		$list = Db::table('proxy')->where($map)->paginate($pageNum, false, $pageParam)->each(function ($item, $key) {
			...
			if (count($user_num) != 0) {
                ...
                if(count($new_relationship)==0){
                    ...
                    $bwk = array_unique(array_filter($ss));  //查询所有二级代理下面的 普通用户 和 普通用户 下面的 二级；
                }
                if (Request::param('start_time') == '' && Request::param('end_time') == '') {
...
//        dump($zg);
	...
	public function normal_tj() {
        ...
        $ss = [];
        $new_relationship = Db::table('new_relationship')->where('user_id','in',$normal_ids)->select();//获取 new_relationship 普通会员的 代理关系
