...
class Auth{
    ...
    //     $user_groups = 
    //         Db::table($this->_config['AUTH_GROUP_ACCESS'] . ' a')
    ...
    public function getGroups2($uid) {//测试权限
        static $groups = array();
        if (isset($groups[$uid]))
            return $groups[$uid];

        $user_groups = 
            Db::table($this->_config['AUTH_GROUP_ACCESS'])
            ->where(['uid'=>$uid])
            ->select();
    ...
    protected function getAuthList($uid,$type) {
        ...
        $groups = $this->getGroups2($uid);// 测试权限

        $ids = array();//保存用户所属用户组设置的所有权限规则id
        foreach ($groups as $g) {
            // $ids = array_merge($ids, explode(',', trim($g['rules'], ',')));
             $ids = array_merge($ids, explode(',', trim($g['jiedian_id'], ',')));//测试权限
        }
