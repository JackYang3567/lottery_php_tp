...
class Backup
{
    ...
    public function fileList()
    {
        ...
        foreach ($glob as $name => $file) {
            if (preg_match('/^\\d{8,8}-\\d{6,6}-\\d+\\.sql(?:\\.gz)?$/', $name)) {
                ...
                $info['kb_size'] = round($file->getSize()/1024);
                $list["{$date} {$time}"] = $info;
            }
        }
        ...
        array_multisort($new_list, SORT_DESC, $list); 
    ...
    public function delFile($time)
    {
        if ($time) {
            ...
            if (count($this->getFile('time', $time))) {
                throw new \Exception("File {$path} 删除失败");
            } else {
                return $time;
            }
        } else {
    ...
    public function backupAll($tables=[],$start=0){
            ...
            foreach($tables as $key=>$value){
                        // 备份表结构
                    if (0 == $start) {
                        $result = $db->query("SHOW CREATE TABLE `{$value}`");
                        $sql = "\n";
                        $sql .= "-- -----------------------------\n";
                        $sql .= "-- Table structure for `{$value}`\n";
                        $sql .= "-- -----------------------------\n";
                    ...
                    //数据总数
                    $result = $db->query("SELECT COUNT(*) AS count FROM `{$value}`");
                    $count = $result['0']['count'];
                    //备份表数据
                    if ($count) {
                        //写入数据注释
                        if (0 == $start) {
                            $sql = "-- -----------------------------\n";
                            $sql .= "-- Records of `{$value}`\n";
                            $sql .= "-- -----------------------------\n";

                           $this->write($sql);
                        }
                        //备份数据记录
                        $result = $db->query("SELECT * FROM `{$value}` LIMIT {$start}, 1000");
                        foreach ($result as $row) {
                            $row = array_map('addslashes', $row);
                            $sql = "INSERT INTO `{$value}` VALUES ('" . str_replace(array("\r", "\n"), array('\\r', '\\n'), implode("', '", $row)) . "');\n";
                            if (false === $this->write($sql)) {
                                return false;
                            }
                        ...
                        if ($count > $start + 1000) {
                            //return array($start + 1000, $count);
                            return $this->backupAll($value, $start + 1000);
                        }
                    }
            }
